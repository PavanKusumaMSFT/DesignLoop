...
const useStyles = makeStyles({
  ...
  card: {
    ...
    // PR10b reflow — at very narrow viewports (≤499px) the 16px card padding
    // stacks on top of the page's already-shrunk 12px L/R padding (PR10a),
    // leaving very little room for the inner table. Drop to 12px so the
    // table scroll wrapper has more usable width.
  ...
  kpiRow: {
    ...
    // PR6 reflow — KPIs wrap to multiple rows when the available width is too
    // small to fit them side by side. Each KPI block keeps its natural width.
    flexWrap: "wrap",
    // PR10b reflow — at narrow viewports KPIs wrap to one-per-row; the 40px
    // gap then becomes wasted vertical space between stacked blocks. Tighten
    // to 16px so the stack feels grouped instead of spaced apart.
    "@media (max-width: 499px)": {
      gap: tokens.spacingHorizontalL,
    },
  },
  // PR6 reflow — horizontal scroll wrapper for the Table at narrow viewports.
  // Standard data-table reflow pattern: keep all columns, scroll inside the
  // wrapper instead of pushing the page wider.
  tableScroll: {
    width: "100%",
    overflowX: "auto",
  },
  // PR6 reflow — force the Table to its natural readable width so the wrapper
  // above scrolls horizontally instead of letting columns squish to illegibility.
  tableMinWidth: {
    minWidth: "880px",
  },
  kpiBlock: {
