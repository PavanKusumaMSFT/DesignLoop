...
const useStyles = makeStyles({
  ...
  pill: {
    ...
    borderRadius: "16px",
    height: "24px",
    // PR10c reflow — explicit `minWidth: 0` overrides the flex default of
    // `min-width: auto` (= intrinsic content width). Without this the pill
    // refuses to shrink below the natural width of "Resource group ==
    // rg-contoso-analytics-canary-03" and overflows the viewport.
    minWidth: 0,
    paddingTop: "0",
    paddingBottom: "0",
    paddingLeft: "10px",
    ...
    lineHeight: "18px",
    // PR10c reflow — cap each pill at the parent flex-wrap container width so
    // a long selected value never pushes the pill past the viewport. Pair
    // with truncation rules on `pillTextGroup` + `pillValue` below.
    maxWidth: "100%",
  ...
  pillTextGroup: {
    ...
    // PR10c reflow — allow this group to shrink within the pill so the value
    // span inside can clip with ellipsis instead of pushing the pill wider
    // than its `maxWidth: 100%` cap.
    minWidth: 0,
    overflow: "hidden",
  },
  pillLabel: {
    ...
    color: tokens.colorNeutralForeground1,
    lineHeight: "18px",
    // PR10c reflow — the filter name and `==` separator must never truncate;
    // only the user-selected value should clip.
    flexShrink: 0,
    whiteSpace: "nowrap",
  },
  pillValue: {
    ...
    color: tokens.colorNeutralForeground1,
    lineHeight: "18px",
    // PR10c reflow — the selected value is the only part of the pill that's
    // allowed to truncate. Single-line ellipsis when the pill hits its width
    // cap; full text otherwise.
    minWidth: 0,
    overflow: "hidden",
