...
import SearchSuggestionPanelBuildMVP from "../../shared/search-suggestion-panel-buildmvp";
...
export const AzureHeaderBuildMVP: React.FC<AzureHeaderBuildMVPProps> = ({
  ...
  useEffect(() => {
    ...
    const animationTimer = setTimeout(() => {
      if (typeof window !== "undefined" && window.innerWidth >= 1680) {
        setIsNavOpen(true);
      }
    }, 100);
  ...
  useEffect(() => {
    if (!initialNavOpen) return; // Only apply responsive behavior when initialNavOpen is true

    const handleResize = () => {
      // Only auto-adjust if user hasn't manually toggled
      if (!userToggledNav) {
        setIsNavOpen(window.innerWidth >= 1680);
      }
    };
