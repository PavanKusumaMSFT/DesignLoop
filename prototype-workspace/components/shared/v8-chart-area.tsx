...
export function AreaChart({
  ...
  const valueExtent = useMemo<[number, number]>(() => {
    if (stacked) {
      const sums = categories.map((_, ci) =>
        series.reduce((sum, s) => sum + (s.data[ci]?.y ?? 0), 0)
      );
  ...
  const classNames = ['ap-chart', 'ap-area-chart', className].filter(Boolean).join(' ');
    ...
    <div className={classNames} style={containerStyle} ref={containerRef}>
      ...
      <div className="ap-chart__body" style={{ display: legendPosition === 'right' ? 'flex' : 'block' }}>
        ...
        >
          ...
          {/* Areas (render in reverse for proper layering when not stacked) */}
          {[...series].map((s, rawSi) => {
            const si = rawSi;
            ...
            const points = s.data.map((d, di) => {
              ...
              const topY = stacked
                ? yScale((stackedData?.[si]?.[di] ?? 0) + d.y)
                : yScale(d.y);
              const baseY = stacked
                ? yScale(stackedData?.[si]?.[di] ?? 0)
                : plotBottom;
              return { cx, topY, baseY, datum: d, index: di };
            });

            const areaPath =
              points.map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.cx} ${p.topY}`).join(' ') +
              ' ' +
              [...points].reverse().map((p, i) => `${i === 0 ? 'L' : 'L'} ${p.cx} ${p.baseY}`).join(' ') +
              ' Z';

            const linePath = points
              .map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.cx} ${p.topY}`)
              .join(' ');

            return (
              <g key={si}>
                <path d={areaPath} fill={color} opacity={fillOpacity} className="ap-area-chart__area" />
                <path d={linePath} fill="none" stroke={color} strokeWidth={2} className="ap-area-chart__line" />
                {showDots &&
                  points.map((p) => (
                    <circle
                      key={p.index}
                      ...
                      onMouseEnter={(e) => {
                        ...
                        showTooltip(
                          { title: String(p.datum.x), items: [{ color, label: s.name, value: formatTickValue(p.datum.y) }] },
                          e.clientX - pt.left,
                          e.clientY - pt.top
                        );
                      }}
